...
static inline uint8_t linearTransform(
    ...
    static constexpr uint8_t linear_transform_table[8][256] = {
    ...
    return
        linear_transform_table[0][a0] ^
        linear_transform_table[1][a1] ^
        linear_transform_table[2][a2] ^
        linear_transform_table[3][a3] ^
        linear_transform_table[4][a4] ^
        linear_transform_table[5][a5] ^
        a6 ^
        linear_transform_table[6][a7] ^
        a8 ^
        linear_transform_table[5][a9] ^
        linear_transform_table[4][a10] ^
        linear_transform_table[3][a11] ^
        linear_transform_table[2][a12] ^
        linear_transform_table[1][a13] ^
        linear_transform_table[0][a14] ^
        a15;
}

static inline SecureBuffer<16> &substitute(SecureBuffer<16> &vector) noexcept {
...
static inline SecureBuffer<16> &inverseSubstitute(SecureBuffer<16> &vector) noexcept {
...
static inline SecureBuffer<16> &L(SecureBuffer<16> &vector) noexcept {
    for (uint8_t i = 0; i < 16; ++i)
        vector[15 - i] = linearTransform(
            vector[(16 - i)  % 16], vector[(17 - i)  % 16], vector[(18 - i)  % 16], vector[(19 - i)  % 16], 
            vector[(20 - i)  % 16], vector[(21 - i)  % 16], vector[(22 - i)  % 16], vector[(23 - i)  % 16],
            vector[(24 - i)  % 16], vector[(25 - i)  % 16], vector[(26 - i)  % 16], vector[(27 - i)  % 16],
            vector[(28 - i)  % 16], vector[(29 - i)  % 16], vector[(30 - i)  % 16], vector[(31 - i)  % 16]
        );
    return vector;
}

static inline SecureBuffer<16> &invL(SecureBuffer<16> &vector) noexcept {
    std::cerr << std::hex;
    for (uint8_t i = 0; i < 16; ++i) {
        vector[i] = linearTransform(
            vector[(i + 1) % 16], vector[(i + 2) % 16], vector[(i + 3) % 16], vector[(i + 4) % 16], 
            vector[(i + 5) % 16], vector[(i + 6) % 16], vector[(i + 7) % 16], vector[(i + 8) % 16],
            vector[(i + 9) % 16], vector[(i + 10) % 16], vector[(i + 11) % 16], vector[(i + 12) % 16],
            vector[(i + 13) % 16], vector[(i + 14) % 16], vector[(i + 15) % 16], vector[i]
        );
    }
    return vector;
}
...
static inline void Feistel(
    ...
    L(substitute(a1 += const_round_key)) += a0;
    a0 = a1_copy;
}
...
SecureBuffer<16> &Kuznechik::decrypt(SecureBuffer<16> &encrypted_text) const noexcept {
...
const SecureBuffer<16> *Kuznechik::getKeySchedule() const noexcept {
