...
public:
    Kuznechik() = default;
    void initKeySchedule(const SecureBuffer<32> &key);
    Kuznechik(const SecureBuffer<32> &key);
    SecureBuffer<16> &encrypt(SecureBuffer<16> &plain_text) const override;
    SecureBuffer<16> &decrypt(SecureBuffer<16> &encrypted_text) const override;
    #ifdef UNIT_TESTS
        const SecureBuffer<16> *getKeySchedule() const;
    #endif
};
